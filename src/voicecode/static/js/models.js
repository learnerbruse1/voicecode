...
function renderManagedModels(data) {
  ...
  modelsListEl.innerHTML = Object.entries(models).map(([name, info]) => {
...
async function waitForManagedModel(modelName, timeoutMs = 240000) {
...
async function downloadManagedModel(modelName) {
  if (!modelName) return;
  showProgress(t("model_downloading"), `${t("model_download_detail")} ${modelName}`);
  const r = await requestJSON("POST", `/models/${encodeURIComponent(modelName)}/download`, {}, {errorTitle: t("model_download_failed"), timeout: 20000});
  ...
  try {
    ...
    showError(t("model_download_failed"), e.message || String(e));
  } finally {
    hideProgress();
  }
}
