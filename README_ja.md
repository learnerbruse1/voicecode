...
各機能と設定の詳しい説明は、[ユーザーガイド](docs/GUIDE_ja.md)（日本語）、[User Guide](docs/GUIDE.md)（English）、[使用指南](docs/GUIDE_zh.md)（简体中文）をご覧ください。
...
- 日本語最適化の Kotoba Whisper v2.0 モデル（日本語では large-v3 より高精度でサイズは約半分）と高速な Distil Whisper Large v3.5 モデルに対応。モデル選択ボタンにはローカライズされたヒントを表示します。
- `127.0.0.1` のみにバインドされ、トークンと Host/Origin 検証で保護されたローカル API。
- 録音中のライブ部分プレビュー（文字起こしの草稿をリアルタイム表示、停止時に確定）。
- クリップボード入力：文字起こし結果をクリップボード経由で貼り付け（キー入力シミュレーションへのフォールバックと遅延設定付き）。
- デコードプリセット：高速 / バランス / 高精度 / カスタム。
- 文字起こし履歴を設定上限に自動トリミング。
- Windows では単一インスタンスで動作し、二重起動時は既存ウィンドウを復元。
- 内蔵のマインスイーパーは初級・中級・上級を搭載し、カードゲームは同梱しません。
...
アプリから追加した依存関係、Whisper モデル、Hugging Face/pip キャッシュは、すべて選択したインストール先に保存されます。設定、ログ、履歴は Windows ユーザーごとの書き込み可能な設定ディレクトリに保存されます。アンインストールではセットアップが管理するプログラムファイルを削除しますが、ユーザーがダウンロードしたモデルなどのランタイムデータは再インストール用に残ります。完全削除する場合は、アンインストール後に残った `runtime` ディレクトリを手動で削除してください。インストール版の静的 UI にはマインスイーパーとその `games.js` のみが含まれます。

v0.2.0 インストーラーは **2026 年 7 月 24 日**に完全な機能検証を行い、**2026 年 7 月 25 日**にマインスイーパーのみを含む構成で再ビルド・再検証しました。今回の再検証ではカスタムパス、3 言語リソース、組み込み pip、カードゲーム削除、アンインストールを確認し、以前の完全検証では上書きインストール、単一インスタンス、キャッシュ済みモデル、文字起こし、再インストールも確認しています。詳細は[最終検証記録](docs/RELEASE_VALIDATION_0.2.0.md)を参照してください。ローカルで検証した成果物は未署名のため、公開前にコード署名と署名後の再検証が必要です。
...
VoiceCode は「読み込み」を表示する前にモデルキャッシュを厳密に検証し、検証済みのローカルスナップショットから直接読み込みます。不完全なダウンロードは部分キャッシュとして扱い、再開できます。エラー詳細のコピー、ローカルポートを保持するウィンドウなし残留プロセスの回復にも対応しました。上部バーには CPU・GPU・メモリの概要のみを表示し、詳細は設定画面に移動しました。マインスイーパーには上級（16 × 30、地雷 99 個）を追加し、新しい UI 文言は英語・簡体字中国語・日本語に対応しています。また、faster-whisper を 1.2.x（Silero VAD v6）へ更新し、日本語最適化の Kotoba Whisper v2.0 モデルと高速な Distil Whisper Large v3.5 モデルを追加、モデル選択ボタンにローカライズされたヒントを表示します。

本フェーズではさらに、録音中のライブ部分プレビュー、クリップボード入力（キー入力フォールバック付き）、デコードプリセット、履歴の自動トリミング、適応的なステータスポーリング、依存状態と Hugging Face エンドポイントのプロセス内キャッシュ、モデル読込後のベストエフォートなウォームアップ（`VOICECODE_SKIP_WARMUP` で無効化）を追加しました。
